function getTimeDifference(dateString) {
    ...
    if (diff < 60000) {
      ...
      return Math.floor(diff / 60000) + ' menit yg lalu';
    } else if (diff < 86400000) {
      return Math.floor(diff / 3600000) + 'jam yg lalu';
    } else {
      return Math.floor(diff / 86400000) + 'hari yg lalu';
    }
  }
...
$(document).delegate(".btn-info-laporan", "click",  function () {
    let modal = $($(this).attr("data-target"));
    let user = $("input[name=id-user]").val();
    $.ajax({
        headers: {
        ...
        success: function (data) {
            ...
            if (data["penugasan"] == null) {
                ...
                $(".admin-message").text(data["penugasan"]["keterangan_admin"]);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                type: "post",
                success: function(data){
                    Swal.fire("Laporan telah dihapus")
                },error: function(err){
                    alert(err.responseText);
                }
