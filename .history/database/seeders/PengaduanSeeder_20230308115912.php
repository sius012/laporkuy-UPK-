    ...
    public function run()
    {
        ...
        for ($i = 0; $i < $jumlahPengaduan; $i++) {
          ...
          //  $pengaduan = new PengaduanController;
            ...
            $pengaduanobj = PengaduanController::tambahGeneral($params);

            if($statusr == "Ke Petugas"){
              //  $penugasan = new PengaduanController();
                ...
                foreach($petugas as $i => $pt){
                    $ptgid[$i] = $pt->id;
                    $ptgj[$i] = $i == 0 ? "Ketua": "Anggota";
                }

                // $params = [
                //     "input"=>[
                //     "id_pengaduan"=>$pengaduanobj->id_pengaduan,
                //     "petugas"=>$ptgid,
                //     "jabatan"=>$ptgj,
                //     "keterangan_admin"=>$contoh_keterangan_admin[rand(0, count($contoh_keterangan_admin)-1)]
                //     ]
                //     ,
                //     "id_admin"=>$admin[rand(0,count($admin)-1)]->id,
                //     "type"=>"request",
                //     "giveMe"=>"result"
                // ];

               // $penugasan->assignpetugasBase($params);
            }
