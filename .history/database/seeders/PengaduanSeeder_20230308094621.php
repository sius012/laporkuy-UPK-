...
class PengaduanSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 0; $i < $jumlahPengaduan; $i++) {
            ...
            $myRequest->request->add(["id_pelapor" => 5]);
            $myRequest->request->add(["judul" => $judul]);
            $myRequest->request->add(["id_jp"=>$jenisp->id_jp]);
            $myRequest->request->add(["keterangan"=>$keterangan])
