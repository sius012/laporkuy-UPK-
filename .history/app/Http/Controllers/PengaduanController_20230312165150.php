...
class PengaduanController extends Controller
{
    ...
    public function assignpetugas(Request $req){
        ...
        if(!$req->has("petugas")){
            Alert::error("Petugas Kosong","isi setidaknya 1 petugas");
            return redirect()->back();
        }
            ...
            try {
              ...
              //Penugasan::where('id_penugasan',$req->id_pengaduan)->update(["keterangan_admin"=>$input["id_pengaduan"]]);
                ...
                ChangeLog::create([
                  ...
                  $notif = Notification::create(["id_user"=>$pengaduan->id_pelapor, "judul"=>"Pengaduan Diverifikasi", "isi"=>"<b>".$pengaduan->judul_pengaduan ."</b> ".":" ."Pengaduan disudah diverifikasi oleh admin"]);
                
                  //notif untuk petugas
                  foreach($input["petugas"] as $j => $ptg){
                        $notif = Notification::create(["id_user"=>$ptg, "judul"=>"Pengaduan Masuk", "isi"=> "<b>".$pengaduan->judul_pengaduan ."</b> ".":" ."Admin menambahkan tugas untuk anda"]);
                  }
    ...
    public function sendtanggapan(Request $req){
        $tanggapan = $this->sendtanggapanBase($req->pesan, $req->id_penugasan);
        if($req->has("no_ajax")){
            return redirect()->back();
        }else{
            return json_encode($tanggapan)
        }
    ...
    public function updatePetugas(Request $req){
        ...
        try {
            ...
            $penugasan = Penugasan::where("id_pengaduan", $req->id_pengaduan)->with("petugas_lapangan")->first();

            //Notif untuk petugas lama
            
            foreach($penugasan->petugas_lapangan as $j => $ptg){
                if(!in_array($ptg->id_petugas, $req->petugas)){
                    $notif = Notification::create(["id_user"=>$ptg->id_petugas, "judul"=>"Penugasan Batal", "isi"=> "<b>".$pengaduan->judul_pengaduan ."</b> ".":" ."Anda sekarang bukan petugas"]);
                }
            }

            foreach($req->petugas as $k => $ptg){
                if(!in_array($ptg, $req->petugas)){
                    $notif = Notification::create(["id_user"=>$ptg, "judul"=>"Penugasan Masuk", "isi"=> "<b>".$pengaduan->judul_pengaduan ."</b> ".":" ."Admin menambahkan tugas untuk anda"]);
                }
            }

            $oldpetugas = $pengaduan->petugas_lapangan;
            ...
            ChangeLog::create([
             ...
             $notif = Notification::create(["id_user"=>$pengaduan->id_pelapor, "judul"=>"Petugas Diperbarui", "isi"=>"<b>".$pengaduan->judul_pengaduan."</b> ".":" ."Petugas telah diperbarui oleh admin"]);
    ...
    public function ubahStatus(Request $req){
        ...
        $notif = Notification::create(["id_user"=>$pengaduan->id_pelapor, "judul"=>"Pengaduan ".$title, "isi"=>$pengaduan->judul_pengaduan .":" .$keterangan]);
    ...
    public function getlampiran(Request $req){
        ...
        $lampiran = Lampiran::where('id_pengaduan',$req->id_pengaduan);

        if($req->jenis != null){
            $lampiran->where('jenis', $req->jenis);
        }
        return json_encode($lampiran->get());
    }
    ...
    public function deletePengaduanBase($id_pengaduan,$keterangan){
        ...
        try {
            $notif = Notification::create(["id_user"=>$datapengaduan->id_pelapor, "judul"=>"Laporan Ditolak", "isi"=>"<b>".$datapengaduan->judul_pengaduan."</b> ".":" .$keterangan]);
